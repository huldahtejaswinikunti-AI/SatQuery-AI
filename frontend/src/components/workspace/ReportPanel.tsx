...
export const ReportPanel: React.FC<ReportPanelProps> = ({
  ...
  const modelCalibration = result?.model_calibration || result?.verified_facts?.model_calibration || result?.trace?.model_calibration;
  ...
  return (
    <div className="w-full h-full rounded border border-[var(--border-hairline)] bg-[var(--bg-panel)] p-6 space-y-6 overflow-y-auto max-h-[760px] transition-all">
      ...
      {/* Notice Banner when Land-Cover Classifier is running on untrained fallback */}
      {modelCalibration === 'untrained_fallback' && (
        <div className="p-4 rounded border border-amber-500/60 bg-amber-950/40 text-amber-200 text-xs font-sans space-y-1.5 shadow-lg shadow-amber-950/20">
          <div className="flex items-center gap-2 font-mono font-bold text-amber-400 text-xs tracking-wide">
            <AlertCircle className="w-4 h-4 text-amber-400 flex-shrink-0" />
            <span>SYSTEM NOTICE: CLASSIFIER UNCALIBRATED (UNTRAINED HEAD)</span>
          </div>
          <p className="text-[11px] text-amber-300/90 leading-relaxed font-sans">
            Observation predictions were generated by an untrained, randomly-initialized classification head because checkpoint <code className="bg-black/50 px-1 py-0.5 rounded text-amber-200 font-mono text-[10px]">models/land_cover/best_model.pt</code> was not detected. Class probabilities are raw uncalibrated artifacts and MUST NOT be used for operational decision-making.
          </p>
        </div>
      )}
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="p-3 rounded bg-[var(--bg-panel-elevated)] border border-[var(--border-hairline)] space-y-1">
              ...
              <span className="font-sans text-[10px] text-[var(--text-secondary)] block truncate">
                {spectral.ndvi_label || 'Spectral Index'}
              </span>
            </div>
            ...
            <div className="p-3 rounded bg-[var(--bg-panel-elevated)] border border-[var(--border-hairline)] space-y-1">
              ...
              <span className="font-sans text-[10px] text-[var(--text-secondary)] block truncate">
                {spectral.ndwi_label || 'Spectral Index'}
              </span>
            </div>
